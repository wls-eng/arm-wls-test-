function usage() { echo "Usage: sh runAdminOfferTest.sh <<< 'basic,cluster,datasource' testpropertyfile testgithubrepourl dsjndi dbtype" 1>&2; exit 1; }
...
do
	...
	"security")
			...
			;;
	"coherence")
			echo "Executing coherence tests"
			echo "sh $testWorkDir/coherence/coherence_test.sh  -i $testInputFileName"
			sudo sh $testWorkDir/coherence/coherence_test.sh  -i $testInputFileName > ${test}.log 2>&1
			;;
			*)
			;;
	esac
